/* Mutex_example_3.c - one rogue thread doesn't use the mutex
   James S. Plank
   April, 2017
 */

/* This code is exact same as mutex_example_1.c, except that the
   last thread doesn't lock or unlock the mutex. */
...
void *share_counter(void *arg)
{
  ...
  while (1) {
    ...
    if (info->id != s->nthreads-1) pthread_mutex_lock(s->lock);      
    s->counter++;
    counter = s->counter;
    ...
    if (info->id != s->nthreads-1) pthread_mutex_unlock(s->lock);
