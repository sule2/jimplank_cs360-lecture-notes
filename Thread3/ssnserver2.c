...
/* This code is very much like ssnserver1.c, except now it will service 
   multiple connections simultaneously.  You'll note, though, there is
   no synchronization among the threads, so there is a race condition
   an the tree, since multiple threads can access and modify it simultaneously. */
